...
/** pass_1()
  * int pass_1(FILE* fptr_source,FILE* fptr_SYMTAB,FILE* fptr_location,char ** label,char ** opcode,char ** operand,char ** opcode_table,int* address,int opcode_table_n,int* location_counter)
  ...
  * opcode table            : char ** opcode_table,int* opcode_address
  * location_count()        : count location, write in symbol table,location.txt */
//void pass_1(FILE* fptr_source,char ** opcode_table,int* opcode_address,int opcode_table_n){
void pass_1(FILE*,char**,int* ,int);
int location_count(FILE* ,FILE* ,char**,char**,char**,char**,int*,int,int*,int);
...
int search_address(char* ,char**,int* ,int );

/** pass_2()
  * void pass_2(char ** label,char ** opcode,char ** operand,int* address,char ** opcode_table,int* opcode_address,int location_counter,int opcode_table_n,int n)
  ...
  * symbol table            : char ** label,char ** opcode,char ** operand,int* address
  * opcode table            : char ** opcode_table,int* opcode_address

  * int location_counter    : last location
  * int n      : number of lines in source code from START to END */
void pass_2(char**,char **,char **,int*,char **,int*,int,int,int);
...
int main(){
    ...
    if ((fptr_source == NULL || fptr_opcode==NULL)){
        ...
        //opcode table
        char** opcode_table = malloc(MAX_LENGTH * sizeof(*opcode_table));
        int* opcode_address = malloc(MAX_LENGTH * sizeof(int));
        for(int i=0;i<MAX_LENGTH;i++){
            opcode_table[i] = malloc(50 * sizeof(char));
        }

        //turn opcode.txt(fptr_opcode) into char opcode_table[MAX_LENGTH][50] , return the number of lines in opcode_table
        size_t len = 0;
        ssize_t read;
        char *line = NULL;
        char* p;
        ...
        while ((read = getline(&line, &len, fptr_opcode) != -1)){
            if(counter>=MAX_LENGTH){
                MAX_LENGTH*=2;
                char** new_opcode_table = realloc(opcode_table, (MAX_LENGTH) * sizeof(*opcode_table));
                int* new_opcode_address = realloc(opcode_address, (MAX_LENGTH) * sizeof(int));
                if (new_opcode_table && new_opcode_address){
                    opcode_table = new_opcode_table;
                    opcode_address = new_opcode_address;
        ...
        int opcode_table_n = counter;

        //pass1, will call pass 2 in it
        pass_1(fptr_source,opcode_table,opcode_address,opcode_table_n);
        fclose(fptr_source);
        fclose(fptr_opcode);
...
int search(char* str,char** table,int n){
    for(int i=0;i<n;i++){
...
void pass_1(FILE* fptr_source,char ** opcode_table,int* opcode_address,int opcode_table_n){
    FILE* fptr_location= fopen("location.txt","w");
    FILE* fptr_SYMTAB = fopen("symbol_table.txt","w");
    if ((fptr_location==NULL || fptr_SYMTAB==NULL)){
        printf("File Not Found");
        return ;
    }
    char** label = malloc(MAX_LENGTH * sizeof(*label));
    char** opcode = malloc(MAX_LENGTH * sizeof(*opcode));
    char** operand = malloc(MAX_LENGTH * sizeof(*operand));
    int* address = malloc(MAX_LENGTH * sizeof(int));
    for(int i=0;i<MAX_LENGTH;i++){
        label[i] = malloc(50 * sizeof(char));
        opcode[i] = malloc(50 * sizeof(char));
        operand[i] = malloc(50 * sizeof(char));
    }
    size_t len = 0;
    ssize_t read;
    char *line = NULL;
    ...
    while ((read = getline(&line, &len, fptr_source) != -1)){
        ...
        if(counter>=MAX_LENGTH){
            MAX_LENGTH*=2;
            char** new_label = realloc(label, (MAX_LENGTH) * sizeof(*label));
            char** new_opcode = realloc(opcode, (MAX_LENGTH) * sizeof(*opcode));
            char** new_operand = realloc(operand, (MAX_LENGTH) * sizeof(*operand));
            int* new_address = realloc(address, (MAX_LENGTH) * sizeof(int));
            if ((new_label && new_address)&&(new_opcode&&new_operand)){
                label = new_label;
                opcode = new_opcode;
                operand = new_operand;
                address = new_address;
                for(int i=counter-1;i<MAX_LENGTH;i++){
                    label[i] = malloc(50 * sizeof(char));
                    opcode[i] = malloc(50 * sizeof(char));
                    operand[i] = malloc(50 * sizeof(char));
                }
    ...
    int  n = counter;
    if(n<=0) return ;
    pass_2(label,opcode,operand,address,opcode_table,opcode_address,location_counter,opcode_table_n,n);

}
int location_count(FILE* fptr_SYMTAB,FILE* fptr_location,char ** label,char ** opcode,char ** operand,char ** opcode_table,int* address,int opcode_table_n,int* location_counter,int i){
...
int search_address(char* str,char** table,int* address,int opcode_table_n){
    for(int i=0;i<opcode_table_n;i++){
        if(strcmp(str,table[i])==0){
            return address[i];
        }
...
void pass_2(char ** label,char ** opcode,char ** operand,int* address,char ** opcode_table,int* opcode_address,int location_counter,int opcode_table_n,int n){
    ...
    */
    FILE* fptr_object_code = fopen("object_code.txt","w");
    FILE* fptr_object_program = fopen("object_program.txt","w");
    if ((fptr_object_code==NULL || fptr_object_program==NULL)){
        printf("File Not Found");
        return ;
    }
    int start=0,lines=0;
